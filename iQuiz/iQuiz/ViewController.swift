...
class ViewController: UITableViewController {
    ...
    var quizes = [quiz]();
    var givenURL = "http://tednewardsandbox.site44.com/questions.json"
    ...
    @IBAction func settingPressed(_ sender: Any) {
        let alertController = UIAlertController(title: "Setting", message: "Please Enter new URL", preferredStyle: .alert)
    ...
    func parseJSON(json: [Dictionary<String, AnyObject>]) {
        for item in json {
            self.quizes.append(quiz(title: item["title"] as! String, desc: item["desc"] as! String, questions: item["questions"] as! [Dictionary<String, AnyObject>]))
        }
    ...
    override func tableView(_ cellForRowAttableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel!.text = quizes[indexPath.row].title
        cell.detailTextLabel!.text = quizes[indexPath.row].desc
